...
- Antes de alterar produção, confirmar que a mudança correspondente está versionada e passou pelo fluxo branch → commit → PR → merge → deploy.
- Mudanças operacionais ou emergenciais feitas diretamente em produção devem ser exceção, ter autorização explícita e ser registradas imediatamente. Quando puderem ser representadas no repositório, criar a mudança equivalente em branch, commit e PR para evitar divergência de configuração.
